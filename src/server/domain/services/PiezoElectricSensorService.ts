...
class PiezoElectricSensorService {
  ...
  private tasksToLock: Promise<any> | null = null
      ...
      ; (async () => {
        while (this.loopService.isStarted) {
          ...
          if (this.store.length === 1000) {
            this.swapStore()
            this.tasksToLock = this.recordRepo.create({
          ...
          if (this.store.length > this.BATCH_DATA_SIZE && this.tasksToLock) {
            await this.tasksToLock
            this.secondaryStore = []
          }
...
export default new PiezoElectricSensorService()
